  ...
  try {
    ...
    if (!!missedPropertiesWarehouse(warehouse)) {
      res
        .status(400)
        .send(
          `Missing/empty required properties in your request body: ${missedPropertiesWarehouse(warehouse).join(', ')}`
        );
      return;
...
const deleteWarehouse = async (req, res) => {
  try {
    const deleted = await knex('warehouses').where({ id: req.params.id }).delete();
